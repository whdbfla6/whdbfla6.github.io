...
모델링의 최종 목적은 관찰하지 않은 데이터 즉 test data에 대한 error를 줄이는 것이다. 허나 대부분의 경우에서 test data를 보유하고 있지 않아 test error를 구할 수 없다. Cross validation은 이미 가지고 있는 데이터를 최대한 활용하는 방법으로 데이터를 나눠서 그 일부를 test data처럼 사용할 것이다.
